...
void RNA_CriarNeuronio(Neuronio* Neuron, int QuantidadeLigacoes)
{
    ...
    {
        Neuron->Peso[i] = rand()%200-100;
        //Neuron->Peso[i] = rand()%2000;
    }
...
void RNA_SalvarRede(RedeNeural* Temp, char* String)
{
    ...
    if(f != NULL)
    {
        ...
        int qtdNeuroniosEntrada = Temp->CamadaEntrada.QuantidadeNeuronios - BIAS;
        int qtdNeuroniosEscondida = Temp->CamadaEscondida[0].QuantidadeNeuronios - BIAS;
        fwrite(&Temp->QuantidadeEscondidas,1,sizeof(int),f);
        fwrite(&qtdNeuroniosEntrada,1,sizeof(int),f);
        fwrite(&qtdNeuroniosEscondida,1,sizeof(int),f);
        fwrite(&Temp->CamadaSaida.QuantidadeNeuronios,1,sizeof(int),f);

        for(k=0; k<Temp->QuantidadeEscondidas; k++)
        {
            for(i=0; i<Temp->CamadaEscondida[k].QuantidadeNeuronios - BIAS; i++)
            {
                for(j=0; j<Temp->CamadaEscondida[k].Neuronios[i].QuantidadeLigacoes; j++)
                {
        ...
        for(i=0; i<Temp->CamadaSaida.QuantidadeNeuronios - BIAS; i++)
        {
            for(j=0; j<Temp->CamadaSaida.Neuronios[i].QuantidadeLigacoes; j++)
            {
...
void RNA_SalvarRedeTerminal(RedeNeural* Temp)
{
    int i,j,k;

    printf("\n");

    printf("%d\n",Temp->QuantidadeEscondidas);
    printf("%d\n",Temp->CamadaEntrada.QuantidadeNeuronios);
    printf("%d\n",Temp->CamadaEscondida[0].QuantidadeNeuronios);
    printf("%d\n",Temp->CamadaSaida.QuantidadeNeuronios);
    ...
    for(k=0; k<Temp->QuantidadeEscondidas; k++)
    {
        for(i=0; i<Temp->CamadaEscondida[k].QuantidadeNeuronios; i++)
        {
            for(j=0; j<Temp->CamadaEscondida[k].Neuronios[i].QuantidadeLigacoes; j++)
            {
                printf("%.2f\t",Temp->CamadaEscondida[k].Neuronios[i].Peso[j]);
            }
        }
        printf("\n");
    }
    ...
    for(i=0; i<Temp->CamadaSaida.QuantidadeNeuronios; i++)
    {
        for(j=0; j<Temp->CamadaSaida.Neuronios[i].QuantidadeLigacoes; j++)
        {
            printf("%.2f\t",Temp->CamadaSaida.Neuronios[i].Peso[j]);
        }
    }
}
